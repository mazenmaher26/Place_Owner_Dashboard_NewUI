...
# =========================
    ...
    ci1.metric("Total Reviews",    total_rev)
    ci2.metric("Positive",         f"{pos_count}",  f"{int(pos_count/total_rev*100)}%" if total_rev else "N/A")
    ci3.metric("Negative",         f"{neg_count}",  f"{int(neg_count/total_rev*100)}%" if total_rev else "N/A")
    ci4.metric("Avg Bot Success",  f"{filtered_df['Bot_Success_Rate'].mean():.1f}%")
...
# =========================
    ...
    op1.metric("Total Orders",      f"{filtered_df['Orders'].sum():,}")
    op2.metric("Avg Daily Orders",  f"{int(filtered_df['Orders'].mean())}")
    op3.metric("Total Calls",       f"{filtered_df['Calls'].sum():,}")
    op4.metric("Avg Bot Success",   f"{filtered_df['Bot_Success_Rate'].mean():.1f}%")
    ...
    with col1:
        ...
        fig_ord = px.area(filtered_df, x="Date", y="Orders",
            color_discrete_sequence=["#2F5C85"], template="plotly_white")
...
# =========================
    ...
    an1.metric("Total Anomalies",    len(place_anomalies))
    an2.metric("🔴 High Severity",   len(high))
    an3.metric("🟡 Medium Severity", len(medium))
