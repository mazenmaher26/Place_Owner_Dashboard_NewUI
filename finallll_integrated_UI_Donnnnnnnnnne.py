...
.kpi-title { font-size: 14px; color: #555555; }
.kpi-value { font-size: 34px; font-weight: bold; color: #000000; }
...
def apply_theme(fig):
    fig.update_layout(paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF", font=dict(color="#1D3143"))
...
with st.sidebar:
    ...
    try:
        resp = requests.post(f"{ANOMALY_API}/detect",
            ...
            timeout=5)
        if resp.status_code == 200 and resp.json()["total_anomalies"] > 0:
            st.error(f"🔴 {resp.json()['total_anomalies']} anomal{'y' if resp.json()['total_anomalies']==1 else 'ies'} detected")
        else:
...
# =========================
    ...
    def fetch_place_anomalies():
        try:
            ...
            resp_d = requests.post(f"{ANOMALY_API}/detect", json={"visits": visits}, timeout=30)
    ...
    if high:
        st.error(f"🔴 **{len(high)} High Severity Anomal{'y' if len(high)==1 else 'ies'} Detected!** Review immediately.")
