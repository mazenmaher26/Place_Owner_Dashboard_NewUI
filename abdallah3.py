...
[data-testid=stHeader] {
 ...
 Sidebar Style Refactor (Blue HeaderStrip Style) 
section[data-testid=stSidebar] {
    background-color #F8F9FB !important;  Make parent clean 
}

section[data-testid=stSidebar]  divfirst-child {
...
div[data-testid=stSidebar] buttonhover {
...
section[data-testid=stSidebar] [data-baseweb=base-input] svg {
 ...
 Sidebar button - make it fully transparentghost 
...
section[data-testid=stSidebar] .stButton  buttonhover {
...
# --- CONFIGURATION ---
BACKEND_BASE_URL = httpsaroundubackend-production.up.railway.appapi

# --- AUTHENTICATION LOGIC ---
def login_user(email, password)
    try
        response = requests.post(
            f{BACKEND_BASE_URL}mobileauthlogin,
...
# --- API HELPERS ---
def get_headers()
    return {Authorization fBearer {st.session_state.get('token')}}
...
def fetch_dashboard_data(start_date, end_date)
    try
        ...
        res = requests.get(f{BACKEND_BASE_URL}ownerdashboard, params=params, headers=get_headers())
...
def fetch_analytics_data(start_date, end_date)
    try
        params = {start_date start_date, end_date end_date}
        res = requests.get(f{BACKEND_BASE_URL}owneranalytics, params=params, headers=get_headers())
...
def fetch_chatbot_stats(start_date, end_date)
    try
        params = {start_date start_date, end_date end_date}
        res = requests.get(f{BACKEND_BASE_URL}ownerchatbot-stats, params=params, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
    except pass
...
def fetch_anomalies()
    try
        res = requests.get(f{BACKEND_BASE_URL}owneranomalies, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
    except pass
    return []
...
def fetch_anomalies_summary()
    try
        res = requests.get(f{BACKEND_BASE_URL}owneranomaliessummary, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
    except pass
    return {}
...
def fetch_opportunities()
    try
        res = requests.get(f{BACKEND_BASE_URL}owneropportunities, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
    except pass
    return []
...
def fetch_my_place()
    try
        res = requests.get(f{BACKEND_BASE_URL}ownermy-place, headers=get_headers())
        if res.status_code == 200 
            data = res.json()
            # Debug log for social links specifically
            # st.sidebar.write(fDEBUG Social FB={data.get('facebook_url')})
            return data
        if res.status_code != 401
            st.error(fBackend returned {res.status_code} {res.text})
        handle_api_error(res)
    except Exception as e
        st.error(fConnection error {e})
    return None

@st.cache_data(ttl=300)
def fetch_categories()
    try
        res = requests.get(f{BACKEND_BASE_URL}mobilecategories)
        if res.status_code == 200 return res.json()
    except pass
    return []
...
def update_place_details(place_id, data)
    try
        res = requests.put(f{BACKEND_BASE_URL}dashboardplaces{place_id}, json=data, headers=get_headers())
        if res.status_code == 200
...
def delete_place_image(image_id)
    try
        res = requests.delete(f{BACKEND_BASE_URL}dashboarduploadimage{image_id}, headers=get_headers())
...
def upload_image(place_id, image_type, file, caption=None)
    try
        ...
        res = requests.post(f{BACKEND_BASE_URL}dashboarduploadplace-image, data=data, files=files, headers=get_headers())
...
def fetch_review_data(start_date, end_date)
    try
        params = {start_date start_date, end_date end_date}
        res = requests.get(f{BACKEND_BASE_URL}ownerreviews, params=params, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
    except pass
...
def fetch_review_list(start_date, end_date)
    try
        params = {start_date start_date, end_date end_date}
        res = requests.get(f{BACKEND_BASE_URL}ownerreviewslist, params=params, headers=get_headers())
        if res.status_code == 200 return res.json()
        handle_api_error(res)
        st.error(fFailed to fetch reviews Backend returned {res.status_code})
    except Exception as e
        st.error(fError fetching reviews {e})
    return []

@st.cache_data(ttl=30)
def fetch_location_data()
    try
        res = requests.get(f{BACKEND_BASE_URL}ownerlocation-heatmap, headers=get_headers())
...
def fetch_interactions_locations()
    try
        res = requests.get(f{BACKEND_BASE_URL}ownerinteractions-locations, headers=get_headers())
        if res.status_code == 200 return pd.DataFrame(res.json())
        handle_api_error(res)
    except pass
    return pd.DataFrame()
...
if selected == Dashboard
    ...
    m1.markdown(fdiv class=kpi-carddiv class=kpi-titleTotal Visitsdiv
    div class=kpi-value{data['visits']}div
    div class=kpi-delta{get_delta_display(data['visits'], prev_data['visits'])}divdiv, unsafe_allow_html=True)

    m2.markdown(fdiv class=kpi-carddiv class=kpi-titlePlace Saveddiv
    div class=kpi-value{data['saves']}div
    div class=kpi-delta{get_delta_display(data['saves'], prev_data['saves'])}divdiv, unsafe_allow_html=True)

    m3.markdown(fdiv class=kpi-carddiv class=kpi-titleDirection Clicksdiv
    div class=kpi-value{data['directions']}div
    div class=kpi-delta{get_delta_display(data['directions'], prev_data['directions'])}divdiv, unsafe_allow_html=True)

    m4.markdown(fdiv class=kpi-carddiv class=kpi-titleCall Clicksdiv
    div class=kpi-value{data['calls']}div
    div class=kpi-delta{get_delta_display(data['calls'], prev_data['calls'])}divdiv, unsafe_allow_html=True)
...
# =========================
    ...
    .anomaly-rowhover { background #f8fafc; }
    .anomaly-rowlast-child { border-radius 0 0 10px 10px; border-bottom none; }
    ...
    st.markdown('div class=anomaly-section-title🚨 AroundU Anomaly Detectiondiv', unsafe_allow_html=True)
...
divspan class=severity-badge {sev_cls}{sev_lbl}spandiv
divspan class=category-pill {cat_cls}{category.capitalize()}spandiv
div
...
div class=priority-dot dot-highdiv
div
div class=priority-label🔴 High Priority ({len(high_items)})div
div class=priority-types{_names(high_items)}div
div class=priority-impactImmediate action required — affects data accuracydiv
div
div, unsafe_allow_html=True)
            ...
            if med_items
                st.markdown(f
div class=priority-block
div class=priority-dot dot-mediumdiv
div
div class=priority-label🟠 Medium Priority ({len(med_items)})div
div class=priority-types{_names(med_items)}div
div class=priority-impactCould be real event or fake visits — needs reviewdiv
div
div, unsafe_allow_html=True)
...
div
div class=priority-label📈 Summarydiv
    ...
    if opportunities
        ...
        for idx, opp in enumerate(opportunities[6])
            ...
            with opp_cols[idx % len(opp_cols)]
                st.markdown(f
div class=priority-block style=border-left4px solid #61A3BB; flex-directioncolumn; gap4px;
...
div, unsafe_allow_html=True)
    ...
    with c2
        ...
        st.markdown(f
        div style=background#F8F9FA; padding20px; border-radius10px; border-left5px solid #2F5C85;
            p style=color#65797E; margin-bottom5px;Total Reviews in periodp
            h2 style=color#1D3143; margin0;{total_reviews}h2
        div
        , unsafe_allow_html=True)
            ...
            div class=review-card
                div class=review-header
                    div
                        ...
                        span style=margin-left10px;color#FFD700;{rev[stars]}span
                    div
...
# =========================
    ...
    if place and categories
        ...
        with st.form(edit_place_form)
            ...
            loc_link = st.text_input(Google Maps Link, placeholder=httpswww.google.commaps...)
        ...
        if place_imgs
            ...
            for idx, img in enumerate(place_imgs)
                with cols[idx % 4]
                    ...
                    st.caption(fPath {img_url}) # Debug label to see why it breaks
                    if img.get(caption)
                        st.caption(img[caption])
                    if st.button(Remove, key=fdel_place_{img['id']}, type=secondary, icon=🗑️)
        ...
        with st.expander(Upload New Menu Photo, expanded=False)
            ...
            menu_caption = st.text_input(Item NameDescription (Optional), key=menu_caption)
            if menu_file
        ...
        if menu_imgs
            ...
            for idx, img in enumerate(menu_imgs)
                with cols[idx % 4]
                    ...
                    st.image(img_url, use_container_width=True)
                    st.caption(fPath {img_url}) # Debug label
                    if img.get(caption)
                        st.caption(img[caption])
                    if st.button(Remove, key=fdel_menu_{img['id']}, type=secondary, icon=🗑️)
                        delete_place_image(img['id'])
        else
