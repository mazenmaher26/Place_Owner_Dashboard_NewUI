...
# =========================
    ...
    # ── KPI row ───────────────────────────────────────────────────
    total_orders = f"{filtered_df['Orders'].sum():,}"
    avg_orders   = f"{int(filtered_df['Orders'].mean())}"
    total_calls  = f"{filtered_df['Calls'].sum():,}"
    bot_success  = f"{filtered_df['Bot_Success_Rate'].mean():.1f}%"

    op1, op2, op3, op4 = st.columns(4)
    with op1:
        st.markdown(f"""<div class="kpi-card"><div class="kpi-title">Total Orders</div>
        <div class="kpi-value">{total_orders}</div>
        <div class="kpi-delta">&nbsp;</div></div>""", unsafe_allow_html=True)
    with op2:
        st.markdown(f"""<div class="kpi-card"><div class="kpi-title">Avg Daily Orders</div>
